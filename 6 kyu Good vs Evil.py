def good_vs_evil(good, evil):
    ...
    total_good, total_evil = sum(i * int(j) for i, j in zip(good_values, good.split())), sum(
        i * int(j) for i, j in zip(evil_values, evil.split()))

    return 'Battle Result: No victor on this battle field' if total_good == total_evil else \
        'Battle Result: Evil eradicates all trace of Good' if total_evil > total_good else 'Battle Result: Good triumphs over Evil'
